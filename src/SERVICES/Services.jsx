...
import { SiTensorflow } from 'react-icons/si'; // Importing an AI-related icon
...
const serviceItems = [
  ...
  {
    route: "/ai_solutions", // New AI service section
    title: 'AI Solutions',
    description: 'Harness the power of AI with our tailored solutions that drive innovation and efficiency.',
    icon: <SiTensorflow className='text-[35px]' />
  },
  {
  ...
  {
    route: "/search_engine_optimization",
    title: 'Search Engine Optimization',
    description: 'Our SEO services help you rank higher in search engines and drive organic traffic to your website.',
    icon: <BiSearchAlt className='text-[35px]' />
  },
  {
    route: "/video_editing",
    title: 'Video Editing',
    ...
    <>
      ...
      <section className='text-white grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6 mt-10 mx-3 xl:mx-10 2xl:mx-32'>
        {serviceItems.map((service, index) => (
          <div key={index} className='shadow-2xl hover:shadow-blue-300 duration-500 border-2 w-full  mx-auto py-4 px-4 hover:border-blue-300 border-gray-600 rounded-xl'>
            <div className='bg-[#4b4b4b] rounded-lg px-8 py-10  bg-opacity-40'>
              <div className='flex flex-col gap-3'>
                {service.icon}
                <h3 className='text-xl font-semibold'>{service.title}</h3>
                <p className='w-full md:w-[300px]'>{service.description}</p>
